...
public class BmmPackageContainerDeserializer {
    public List<PersistedBmmPackage> deserialize(CompositeOdinObject packageContainerDefinition) {
        ...
        List<OdinObject> packageDefinitions = packageContainerDefinition.getKeyedObjects();
        packageDefinitions.forEach(packageDefinition -> {
            BmmPackageDeserializer deserializer = new BmmPackageDeserializer();
            PersistedBmmPackage bmmPackage = deserializer.deserialize((CompositeOdinObject)packageDefinition);
            bmmPackages.add(bmmPackage);
        });
        return bmmPackages;
    }
}
