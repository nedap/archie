...
public class BmmEnumerationIntegerSerializer extends BaseBmmSerializer<PersistedBmmEnumerationInteger> {

    /**
     * No-arg constructor.
     */
    public BmmEnumerationIntegerSerializer() {
    }
     ...
     */
    public BmmEnumerationIntegerSerializer(PersistedBmmEnumerationInteger persistedModel) {
    ...
    public void serialize(int indentationCount, StringBuilder builder) {
        ...
        indentByAndAppend(builder, indentCount, OdinSerializationUtils.buildKeyedObjectOpeningDeclarationWithCast(getPersistedModel().getName(), PersistedBmmEnumerationInteger.P_BMM_ENUMERATION_INTEGER));
        ++indentCount;
        ...
        if(getPersistedModel().getItemValues().size() > 0) {
            indentByAndAppend(builder, indentCount, OdinSerializationUtils.buildOdinIntegerListPropertyInitialization(BmmConstants.BMM_PROPERTY_ITEM_VALUES, getPersistedModel().getItemValues()));
        }
        --indentCount;
