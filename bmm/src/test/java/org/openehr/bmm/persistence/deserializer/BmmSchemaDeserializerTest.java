...
//        assertEquals("2.0", persistedBmmSchema.getBmmInternalVersion());
    ...
    public void createClasses() {
        ...
        for(PersistedBmmClass clazz:persistedBmmSchema.getClassDefinitions()) {

            System.out.print(String.format("public class %s", clazz.getName()));
            if(clazz.getAncestors() != null && clazz.getAncestors().size() == 1) {
                System.out.print(" extends " + clazz.getAncestors().get(0));
            } else if(clazz.getAncestors().isEmpty()) {
                //no ancestors. common root object?
            } else {
            ...
            for(PersistedBmmProperty property:clazz.getProperties().values()) {
                ...
                if(property instanceof PersistedBmmSingleProperty) {
                    PersistedBmmSingleProperty p = (PersistedBmmSingleProperty) property;
                    System.out.print("private " + toJavaType(p.getType()) + " " + property.getName());
                    System.out.println(";");
                } else if(property instanceof PersistedBmmContainerProperty) {
                    PersistedBmmContainerProperty p = (PersistedBmmContainerProperty) property;
    ...
    private String toJavaContainerType(String type) {
        switch(type) {
            case "List":
