...
fragment ARCHETYPE_HRID_ROOT : (NAMESPACE '::')? IDENTIFIER '-' IDENTIFIER '-' IDENTIFIER '.' CONCEPT_ID_IDENTIFIER ;

fragment NAMESPACE  : LABEL ('.' LABEL)+ ;
fragment LABEL      : ALPHA_CHAR ( NAME_CHAR* ALPHANUM_CHAR )? ;
fragment IDENTIFIER : ALPHA_CHAR WORD_CHAR* ;
fragment CONCEPT_ID_IDENTIFIER : ALPHA_CHAR (WORD_CHAR)*;
fragment VERSION_ID : DIGIT+ (DOT_SEGMENT*) (('-rc' | '-alpha') DOT_SEGMENT? )? ;
